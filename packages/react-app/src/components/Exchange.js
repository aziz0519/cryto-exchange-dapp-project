...
import { findPoolByTokens, getAvailableTokens, getCounterpartTokens, getFailureMessage, getSuccessMessage, isOperationPending } from '../utils'
...
const Exchange = ({ pools }) => {
    ...
    // approve initiating a contract call
    const { state : swapApproveState, send: swapApprovedSend } =
        useContractFunction(fromTokenContract, "approve", {
            transactionName: "onApproveRequested",
            gasLimitBufferPercentage: 10,
        });
    // swap iniitiating a contract call
    const { state: swapExecuteState, send: swapExecuteSend } =
        useContractFunction(routerContract, "swapExactTokensfromTokens", {
